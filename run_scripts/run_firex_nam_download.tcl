...
###############################################################################
#
## Test forecast for FIREX-AQ, by Xinxin Ye -2019
#
#working directory 
set HOME_DIR /nobackup/ctrujil1/WRF/Tracer_Code/runs/XinxinCode
...
set 1proc_cmd "qsub -N matlab -select=1:ncpus=1:model=bro -l walltime=01:00:00"
...
set fnl "no"   
set gfs "no"
# To use NAM data for neteorology, set nam to yes 
...
if { $data1 > 0} {
   ...
   set secondi_aux [clock seconds]
   #plus one day 
   set secondi [expr $secondi_aux + 3600*24]
...
set flog [file join $SCRIPT_DIR run_download_[clock format [clock seconds] -format "%Y-%m-%d_%H:%M:%S" ]\.log]
set log [open $flog w+]
file attributes $flog -permissions a+r
...
puts $log "# ---------------------------------------------------------"
  ...
  foreach hour [exec seq 0 24 $timestep_tracer ] {
      ...
      flush $log
      cd $SCRIPT_DIR
      set c [catch { eval "exec $SCRIPT_DIR/download_maiac_nrt.sh ${date_str} ${maiac_dir}" } msg ]
      puts $log $msg
      flush $log
  }
}
