...
#
#working directory 
set HOME_DIR /nobackup/ctrujil1/WRF/Tracer_Code/runs/XinxinCode; #/home/xye/firechem/FIREX-AQ
...
set gfs_anl_dir [file join $HOME_DIR input/GFS_anl]
set gfs_fst_dir [file join $HOME_DIR input/GFS_fst]
...
set CT_EMIS_DIR [file join $HOME_DIR CT_Emis]

set 1proc_cmd "qsub -N matlab -l select=1:ncpus=1:model=bro -l walltime=01:00:00 -m abe -M corey.trujillo@colorado.edu"
set qsub "qsub"
set ncks ncks; #"/usr/local/other/SLES11.3/nco/4.6.8/gcc-5.3-sp3/bin/ncks"
...
set wps "yes";			# "yes"
set real "yes";          # "yes"
set wait_qfed "no";           # "yes"
#set fire_qfed2 "no";          #"yes"  
set fire_qfed2_hyb "yes"
#set fire_qfed2_CT "no"
set rst_ic "no";           #"yes"
set wrf_part1 "yes";           #"yes"
set wrf_part2 "no";          # "yes"
set wrf_part3 "no";           #"yes"
...
# Remove old logs and define procedure's new log file
set old_log [glob -nocomplain RUN_2019-* ]
if { $old_log > 0 } {eval "exec mv $old_log trash/"}
...
# Trujillo Download GFS
if { $truj_gfs == "yes" } {
	puts $log "# ---------------------------------------------------------"
	puts $log "# Download GFS forecast data from NCEP data archive "
	puts $log "# ---------------------------------------------------------"
	# forecast cycle: 00 UTC  
	set path_gfs_fst [file join $gfs_fst_dir $week\_00]  
#	set path_anl [file join $gfs_anl_dir ]
if { ![file exists $path_gfs_fst]} {file mkdir $path_gfs_fst; file attributes $path_fst -permissions a+rx;  puts $log "directory $path_gfs_fst made"}
if { ![file exists $gfs_anl_dr]} {file mkdir $gfs_anl_dir; file attributes $gfs_anl_dir -permissions a+rx;  puts $log "directory $gfs_anl_dir made"}

# download forecast data 
set c [catch {eval "exec $SCRIPT_DIR/download_gfs.sh ${week}00 ${timestep_nam_fst} 3 ${path_gfs_fst}"} msg ]
puts $log $msg
flush $log 
...
# download MAIAC
##############################
# Works interactively on pfe's but not compute nodes.
##############################
if { $maiac == "yes" } {
puts $log "# ---------------------------------------------------------"
...
# download NAM data
##############################
# Works interactively on pfe's but not standard nodes. 
##############################
if { $nam == "yes" } {
puts $log "# ---------------------------------------------------------"
...
#run WPS 
##############################
# Successful Interactive run, needs to be tested with qsub 
##############################
if { $wps == "yes" } {
puts $log "# ---------------------------------------------------------"
...
#  if {$c == 1 && $msg != 0} {puts $log "[clock format [clock seconds] -format "%H:%M:%S" ] |E| Standard Error link_grib.csh:\n$msg"; END_shell 1}
  ...
  # link grib 
  set c [catch { eval "exec ./link_grib.csh [file join grib_files 20]" } msg ]
...
#  exec ln -sf ungrib/Variable_Tables/Vtable.GFS_new Vtable
  ...
  # run mod_levs 
  set command "exec qsub ./run_mod_levs.sh"
...
# wait to make sure QFED data have been updated 
if { $wait_qfed == "yes" } { 
  puts $log "Waiting: wait until 00:35 EDT to continue ..."
  flush $log
  set c [catch {eval "exec date -d \"$saaaa-$smm-$sdd 00:35:00\"  +%s"} msg0 ]
  set c [catch {eval "exec date +%s"} msg ]
  while { $msg< $msg0 } {
        exec sleep 3
        set c [catch {eval "exec date +%s"} msg ]
    }
  puts $log "Time to download QFED2 data!"
  flush $log 
}

# hybrid Trujillo/Ye fire emissions process:
if { $fire_qfed2_hyb == "yes" } {
puts $log "# ---------------------------------------------------------"
...
flush $log
  ...
  # Download the emission files from a remote server until the latest day, then link current day emissions to following forecast days 
  ...
  flush $log
	...
	if {[file exists [file join $CT_EMIS_DIR wrfinput_d01]]} {eval "file delete -force [file join $CT_EMIS_DIR wrfinput_d01]"}
	set c [catch {eval "exec ln -s $WRF_TRACER_DIR/wrfinput_d01 $CT_EMIS_DIR" } msg ]

	cd [file join $CT_EMIS_DIR QFED qfed_to_finn]
	exec cp format_QFED_into_FINN_PM10.m format_QFED_into_FINN_PM10.sed
	set sedcommand [open sedcommand.sed w+]
	puts $sedcommand "/date_i=/c\\date_i=\[$fire_aaaa $fire_mm $fire_dd $fire_hh 0 0\]"
	puts $sedcommand "/date_f=/c\\date_f=\[$eaaaa $emm $edd $ehh 0 0\]"
	puts $sedcommand "/path_qfed=/c\\path_qfed='$FIRE_QFED2_FILE_DIR/'"
	close $sedcommand
	...
	puts $log "# ---------------------------------------------------------"
	puts $log "# format_QFED_into_FINN_PM10.m edited"
	puts $log "# ---------------------------------------------------------"
#	puts $log "# Running format_QFED_into_fin_PM10.m...."
#	puts $log "# ---------------------------------------------------------"
	flush $log

	set old_qfed_txt [glob -nocomplain QFED_in_FINN_format_pm10_*]
	if { $old_qfed_txt > 0 } {eval "exec rm $old_qfed_txt"}
	puts $log "# ---------------------------------------------------------"
	puts $log "# Old QFED_in_FINN file removed"
	puts $log "# ---------------------------------------------------------"
	flush $log
#	set c [catch { eval "exec /nasa/matlab/2017b/bin/matlab -nodisplay -nodesktop -nosplash -r \"try;format_QFED_into_FINN_PM10;catch;disp('MATLAB Problem');exit;end\"" } msg ]
#	puts $log $msg
#	puts $log "Finished running format_QFED_into_fin_PM10.m"
#	flush $log
...
#	flush $log
	...
	cd [file join $CT_EMIS_DIR QFED finn_to_wrffire]
##	if { $fire_dd < $edd } { set finn_edd [expr $fire_dd + 1] } else { set finn_edd $edd }

	exec cp Header_fire_emis Header_fire_emis.sed
	set sedcommand [open sedcommand.sed w+]
	puts $sedcommand "/start_date/c\\start_date       ='$fire_aaaa-$fire_mm-$fire_dd',"
	puts $sedcommand "/end_date/c\\end_date         ='$eaaaa-$emm-$edd',"
	puts $sedcommand "/fire_filename/c\\fire_filename(1)  = 'QFED_in_FINN_format_pm10_$fire_aaaa$fire_mm$fire_dd\_$eaaaa$emm$edd.txt'"
	close $sedcommand
	exec sed -f sedcommand.sed Header_fire_emis.sed > Header_fire_emis
	file delete -force sedcommand.sed Header_fire_emis.sed

	puts $log "# ---------------------------------------------------------"
	puts $log "# Header for fire_emis.exe edited "
	puts $log "# ---------------------------------------------------------"
	flush $log

	puts $log "# ---------------------------------------------------------"
	puts $log "# Submitting QFED job..."
	puts $log "# ---------------------------------------------------------"
	flush $log
...
#	flush $log
	...
	cd [file join $CT_EMIS_DIR QFED]
	set qfed2finn [file join $CT_EMIS_DIR QFED finn_to_wrffire ]
	set finn2wrffire [file join $CT_EMIS_DIR QFED finn_to_wrffire] 

#	set c [catch {eval "exec qsub -v qfed=$qfed2finn, finn=$finn2wrffire RunQFED.sh "} msg]
	set c [ catch {eval "exec qsub RunQFED.sh"} msg ]
	puts $log $msg
	flush $log
#	set c [catch {eval "exec qsub \" RunQFED.sh [file join $CT_EMIS_DIR QFED qfed_to_finn]  [file join $CT_EMIS_DIR QFED finn_to_wrffire]' "} msg ]
	while { $c==0 } {
        exec sleep 15
        set c [catch {eval "exec qstat -u ctrujil1 | grep QFED"} status ]
		if { $status > 0 } { puts $log "queue status: $status" }
		flush $log
    }
  ...
  set fire_f [glob -nocomplain [file join $CT_EMIS_DIR QFED finn_to_wrffire wrffirechemi_d*]]
   foreach x $fire_f {
    exec mv $x $FIRE_OUT_week/
  }
  puts $log "Moved wrffirechemi_d01* to $FIRE_OUT_week"
  flush $log
  ...
  foreach hour [exec seq 0 1 $timestep_tracer ] {
	  ...
	  puts $log "time_str: $time_str"
	  puts $log "stripped: $stripped"
  ...
  puts $log "# ---------------------------------------------------------"
  puts $log "# Submitting Tracer Job..."
  puts $log "# ---------------------------------------------------------"
  flush $log 
...
if {$skip_day==5} {
  ...
  puts $log $msg
  flush $log
  puts $log "Waiting: replace nofire wrfinput ... "
  set c [catch {eval "exec qstat -u ctrujil1 | grep matlab"} status ]
  while { $c==0 } {
        exec sleep 5
        set c [catch {eval "exec qstat -u ctrujil1 | grep matlab"} status ]
		puts $log "queue status: $status"
		flush $log
    }
...
flush $log
  ...
  cd $SCRIPT_DIR
  set c [catch { eval "exec $SCRIPT_DIR/run_wrf_tracer_inversion_part2_daily.sh $wrf_aaaa $wrf_mm $wrf_dd $wrf_hh $timestep_tracer $timestep_nofire $timestep $timestep_8tracer ${WRF_TRACER_DIR} ${WRF_NOFIRE_DIR} ${WRF_ANALYSIS_DIR} ${WRF_TRACER_OUT_week} ${WRF_NOFIRE_OUT_week} ${WRF_ANALYSIS_OUT_week} ${FIRE_OUT_week} ${FIRE_ANA_OUT_week} ${FIRE_ANA_TRACER_OUT_week} ${anthro_emis_dir} ${WRF_TRACER_DIR} ${maiac_dir} ${MAIAC_SCRIPT} ${MATLAB_TRACER_DIR}  ${JOINER_DIR} 68 8" } msg ]
  puts $log $msg
  flush $log
}
...
flush $log
  ...
  set c [catch { eval "exec $SCRIPT_DIR/run_wrf_tracer_inversion_part3_daily.sh $wrf_aaaa $wrf_mm $wrf_dd $wrf_hh $timestep_tracer $timestep_nofire $timestep $timestep_8tracer ${WRF_TRACER_DIR} ${WRF_NOFIRE_DIR} ${WRF_ANALYSIS_DIR} ${WRF_TRACER_OUT_week} ${WRF_NOFIRE_OUT_week} ${WRF_ANALYSIS_OUT_week} ${FIRE_OUT_week} ${FIRE_ANA_OUT_week} ${FIRE_ANA_TRACER_OUT_week} ${anthro_emis_dir} ${WRF_TRACER_DIR} ${SCRIPT_DIR} ${MATLAB_TRACER_DIR} ${maiac_dir} ${MAIAC_SCRIPT} ${JOINER_DIR} 68 8" } msg ]
  puts $log $msg
  flush $log
