...
set 1proc_cmd "/usr/slurm/bin/sbatch -J matlab --ntasks=1 --cpus-per-task=1 --time=01:00:00 -o output.%j -e error.%j --account=s1930 --reservation=firex-aq" 
set squeue "/usr/slurm/bin/squeue"
...
#  exec ln -sf ungrib/Variable_Tables/Vtable.GFS_new Vtable
  ...
  # run mod_levs 
  set command "exec /usr/slurm/bin/sbatch ./run_mod_levs.sh"
  set c [catch { eval $command } msg ] 
  puts $log "Waiting: mod_levs.exe"
  set c [catch {eval "exec $squeue -u xye | grep modlev"} status ]
  while { $c==0 } {
        exec sleep 5
        set c [catch {eval "exec $squeue -u xye | grep modlev"} status ]
    }
  ...
  # run metgrid 
  set command "exec /usr/slurm/bin/sbatch ./run_metgrid.csh"
  set c [catch { eval $command } msg ] 
  puts $log "Waiting: metgrid.exe"
  set c [catch {eval "exec $squeue -u xye | grep metgrid"} status ]
  while { $c==0 } {
        exec sleep 5
        set c [catch {eval "exec $squeue -u xye | grep metgrid"} status ]
    }
...
#  set c [catch {eval "exec ln -sf wrffirechemi_d01_$wrf_aaaa\-$wrf_mm\-$wrf_dd\_$wrf_hh\:00:00 wrffirechemi_d01" } msg ]  
  ...
  # Submitt
 cd $WRF_TRACER_DIR
  set c [catch { eval "exec /usr/slurm/bin/sbatch $WRF_TRACER_DIR/run_real.csh"} msg ]
  puts $log $msg
  #check wrfinput-files are generated
  puts $log "Waiting: real.exe ... "
  set c [catch {eval "exec $squeue -u xye | grep real"} status ]
  while { $c==0 } {
        exec sleep 5
        set c [catch {eval "exec $squeue -u xye | grep real"} status ]
    }
...
flush $log
  ...
  set c [catch {eval "exec $squeue -u xye | grep matlab"} status ]
  while { $c==0 } {
        exec sleep 5
        set c [catch {eval "exec $squeue -u xye | grep matlab"} status ]
    }
  ...
  set c [catch { eval "exec /usr/slurm/bin/sbatch \"run_reserve.csh\" >& fire_emis.log" } msg ]
  puts $log $msg
  puts $log "Waiting: fire_emiss ... "
  set c [catch {eval "exec $squeue -u xye | grep fire"} status ]
  while { $c==0 } {
        exec sleep 5
        set c [catch {eval "exec $squeue -u xye | grep fire"} status ]
    }
  ...
  puts $mfile  "exit;"
  close $mfile 
  set c [catch {eval "exec /usr/slurm/bin/sbatch \"create_tracer_emissions.sh\" "} msg ]
  puts $log $msg

  puts $log "Waiting: create 68 tracers ..."
  flush $log 
  set c [catch {eval "exec $squeue -u xye | grep etracer"} status ]
  while { $c==0 } {
        exec sleep 5
        set c [catch {eval "exec $squeue -u xye | grep etracer"} status ]
    }
